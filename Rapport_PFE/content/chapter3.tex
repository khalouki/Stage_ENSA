...
Sur le plan technique, la détection d’anomalies utilise un modèle \textit{Isolation Forest}. L’estimation du risque de panne ou de maintenance s’appuie sur un \textit{Random Forest Classifier}. Le score de santé, le niveau de risque et les recommandations sont complétés par une logique par règles, afin de rendre les résultats exploitables même lorsque les modèles entraînés ne sont pas disponibles ou lorsque des seuils métier doivent être pris en compte.

Les caractéristiques analysées sont \texttt{temperature}, \texttt{vibration}, \texttt{usage\_duration} convertie en \texttt{usage\_hours}, \texttt{motor\_speed} et l’historique des erreurs. Les sorties exposées au tableau de bord et aux endpoints d’analyse sont le statut d’anomalie, la probabilité indicative de panne, le score de santé, le niveau de risque et une recommandation de maintenance.
