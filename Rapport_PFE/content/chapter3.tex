...
Le projet est organisé en deux parties principales : le frontend et le backend. Cette organisation facilite la maintenance et permet d’isoler les responsabilités.
...
\hline
\texttt{backend/app/core} & Configuration centrale, sécurité, gestion des paramètres et services techniques partagés. \\
\hline
...
\hline
\texttt{backend/app/schemas} & Schémas de validation et de sérialisation des données échangées avec l’API. \\
\hline
\texttt{backend/data} & Jeux de données utilisés pour la simulation, l’entraînement et la démonstration. \\
\hline
\texttt{backend/ml\_models} & Modèles entraînés et métadonnées utilisés par les modules d’analyse et de prédiction. \\
\hline
\texttt{backend/scripts} & Scripts de simulation MQTT et d’entraînement des modèles de Machine Learning. \\
\hline
\texttt{backend/tests} & Tests automatisés couvrant les fonctionnalités backend principales. \\
\hline
\texttt{backend/docs} & Documentation technique complémentaire liée aux modules backend. \\
\hline
\end{tabular}
\end{table}
...
L’inscription publique crée uniquement des comptes étudiants. Elle est limitée aux adresses institutionnelles de l’Université Sultan Moulay Slimane dont le domaine est exactement \texttt{usms.ac.ma}. Cette restriction améliore le contrôle d’accès à l’espace étudiant, mais elle ne constitue pas à elle seule une preuve de possession de l’adresse. Une vérification par lien de confirmation ou par authentification institutionnelle SSO pourra être ajoutée ultérieurement.
