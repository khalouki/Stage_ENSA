...
La plateforme réalisée repose sur un backend FastAPI, une interface Next.js et des modules spécialisés pour la visualisation 3D, l’interprétation des fichiers de fabrication et l’analyse des données de télémétrie.
...
\begin{itemize}
    ...
    \item un type découpeuse laser prévu pour l’extension de la plateforme.
\end{itemize}
...
La télémétrie est simulée par un script Python qui publie périodiquement des messages MQTT. Chaque message est envoyé sur un topic de la forme :

\begin{center}
\texttt{fablab/machines/\{machine\_id\}/data}
\end{center}

Le message contient l’identifiant de la machine, la température, la vibration, la durée d’utilisation, la vitesse du moteur, une éventuelle erreur et un horodatage. Le backend s’abonne au motif \texttt{fablab/machines/+/data}, parse les messages JSON et enregistre les mesures dans la base de données.
...
\section{Tableau de bord Data Science}
...
Le module Data Science exploite les mesures reçues afin de produire :

\begin{itemize}
    \item un statut d’anomalie par machine ;
    \item un score de santé sur 100 ;
    \item un niveau de risque de maintenance ;
    \item une probabilité indicative de panne ;
    \item une recommandation de maintenance ;
    \item des facteurs explicatifs tels que température, vibration, utilisation et erreurs récentes.
\end{itemize}
...
\caption{Tableau de bord administrateur et monitoring intelligent}
\label{fig:screenshot-dashboard-ds}
\end{figure}

\section{Alertes d’anomalies et score de santé}

La détection des anomalies repose sur deux niveaux. Le premier utilise des règles et des seuils adaptés au type de machine, par exemple pour la température, la vibration ou la vitesse du moteur. Le second exploite les modèles entraînés avec scikit-learn lorsque les fichiers de modèles sont disponibles.

Le score de santé est calculé à partir du risque estimé. Un score élevé indique une machine considérée comme saine, tandis qu’un score faible signale un besoin de surveillance ou d’intervention. Les cartes de prédiction du tableau de bord permettent de consulter rapidement les machines présentant le risque le plus important.
...
\section{Graphiques et historique de télémétrie}
