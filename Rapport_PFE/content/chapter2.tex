...
Le système doit également être capable de recevoir des données simulées issues des machines, de les stocker et de les exploiter pour la détection d’anomalies et l’aide à la maintenance.
...
\begin{itemize}
    ...
    \item \textbf{Module Data Science} : détection d’anomalies, estimation du score de santé, niveau de risque et recommandations.
...
La télémétrie est reçue sous forme de messages JSON publiés via MQTT. Le backend associe chaque message à une machine existante, enregistre les mesures et expose l’historique à travers les routes de monitoring.

\subsection{Conception de l’analyse intelligente}

Le module Data Science combine une analyse par règles et des modèles entraînés avec scikit-learn. Il produit un statut d’anomalie, un score de santé, une probabilité de panne indicative et une recommandation de maintenance.
...
\techitem{MQTT}{images/mqtt.png}{MQTT est un protocole léger de communication publish/subscribe très utilisé dans les systèmes IoT. Dans ce projet, il permet de simuler l’envoi de mesures machines vers le backend. Les messages sont publiés sur des topics propres à chaque machine, selon le motif suivant :
...
\newcommand{\iotitem}[3]{%
  ...
  \begin{center}
    \IfFileExists{#2}
    {\includegraphics[width=0.28\textwidth,height=2.2cm,keepaspectratio]{#2}}
    {\fbox{\parbox[c][2.2cm][c]{0.28\textwidth}{\centering\scriptsize #1\\\texttt{\detokenize{#2}}}}}
  \end{center}
  #3
  \par\vspace{0.35cm}
}
...
Cette partie présente les composants matériels et les équipements de fabrication numérique liés à la dimension IoT du FabLab. Ces éléments permettent de relier la plateforme virtuelle à un environnement physique réel, en assurant la collecte de mesures, la communication des données et la supervision des machines.

% TODO: ajouter l'image correspondante dans le dossier images : images/esp8266.png
...
\iotitem{Protocole MQTT}{images/mqtt.png}{MQTT est un protocole de messagerie léger basé sur le modèle publication/abonnement. Il est adapté aux systèmes IoT, car il réduit la charge réseau et facilite l’échange de données entre capteurs, microcontrôleurs et applications backend. Dans ce projet, il joue le rôle de canal de communication pour la télémétrie des machines. Les mesures peuvent être publiées sur des topics dédiés, puis consommées par la plateforme afin d’assurer le suivi et l’analyse des équipements.}

% TODO: ajouter l'image correspondante dans le dossier images : images/imprimante_3d.png
...
% TODO: ajouter l'image correspondante dans le dossier images : images/cnc.png
