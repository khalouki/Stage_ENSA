...
Les métriques disponibles concernent le classifieur NLP d'intentions. Le fichier de rapport du classifieur indique un jeu de 124 questions étiquetées, séparé en 86 exemples d'entraînement, 19 exemples de validation et 19 exemples de test.
...
\hline
Taille de l'échantillon & 19 & 19 \\
\hline
\end{tabular}
\end{table}

Ces résultats doivent être interprétés avec prudence, car le jeu de test est réduit. Ils montrent surtout que le pipeline d'évaluation existe et que les métriques sont sauvegardées, mais ils ne suffisent pas pour conclure à une robustesse linguistique élevée.
...
\hline
Classifieur NLP & Accuracy, précision, rappel, F1-score et matrice de confusion. & Enrichissement du corpus, formulations multilingues et validation sur davantage de questions. \\
\hline
...
Le jeu de données NLP reste limité et les métriques actuelles montrent une performance modeste sur un petit échantillon de test. Le système fonctionne comme un classifieur d'intentions supervisé, pas comme un agent conversationnel génératif. Les réponses sont déterministes et fondées sur les données disponibles.
